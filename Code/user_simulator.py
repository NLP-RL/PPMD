...
class UserSimulator:
    ...
    def reset(self):
        ...
        self.info = {}
        self.reject  = 0
        self.chh.update(self.goal['inform_slots'])
        self.chh.update(self.goal['fs'])
    ...
    def step(self, agent_action,stp):
        ...
        for value in agent_action['inform_slots'].values():
            assert value != 'UNK'
            if(agent_action['intent']!='Persuassion'):
        ...
        if agent_action['round'] == self.max_round:
            ...
            elif agent_intent == 'Persuassion':
                self._response_to_Persuassion(agent_action)
            elif agent_intent == 'Repersuassion':
                self._response_to_Repersuassion(agent_action)
...
#            
        ...
        print('fff:',self.state['inform_slots'])
    ...
    def _response_to_Persuassion(self,agent_action):
        r = random.randint(0,1)
        if(r<0.5 and len(self.state['rest_slots'])<=1 ):
            self.state['intent']='done'
            self.constraint_check = SUCCESS
        elif(r<0.5 and len(self.state['rest_slots'])>1):
            if(len(self.info)):
                key,val = random.choice(list(self.info.items()))
    ...
    def _response_to_Repersuassion(self,agent_action):
        r = random.randint(0,1)
        if(r<0.5 and len(self.state['rest_slots'])<=1):
            self.state['intent']='done'
            self.constraint_check = SUCCESS
    ...
    def _response_to_UserGoalUpdate(self,agent_action):
        ...
        if(len(self.state['rest_slots'])>1):
            ...
            key_choice, value_choice = random.choice(list(self.goal['fs'].items()))
            self.state['inform_slots'][key_choice] = value_choice
            self.chh.pop(key_choice,None)
    ...
    def _response_to_done(self,agent_action):
        ...
        if(self.state['history_slots'][self.default_key] == 'no match available'):
            self.constraint_check == FAIL
            return self._response_to_UserGoalUpdate(agent_action)
